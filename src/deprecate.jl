...
for T in (:WithinRange, :NeighborNumber)
    @eval function RecurrenceMatrix{$T}(x::AbstractMatrix, ε; kwargs...)
        @warn string("`RecurrenceMatrix{", $T, "}(x::AbstractMatrix, ε; kwargs...)` is deprecated, use `RecurrenceMatrix{", $T, "}(StateSpaceSet(x), ε; kwargs...)`")
        RecurrenceMatrix{$T}(StateSpaceSet(x), ε; kwargs...)
    end

    for call in (:CrossRecurrenceMatrix, :JointRecurrenceMatrix)
        @eval function $call{$T}(x::AbstractMatrix, y, ε; kwargs...)
            @warn string("`", $call, "{", $T, "}(x::AbstractMatrix, y, ε; kwargs...)` is deprecated, use `", $call, "{", $T, "}(StateSpaceSet(x), y, ε; kwargs...)`")
            $call{$T}(StateSpaceSet(x), y, ε; kwargs...)
        end

        @eval function $call{$T}(x, y::AbstractMatrix, ε; kwargs...)
            @warn string("`", $call, "{", $T, "}(x, y::AbstractMatrix, ε; kwargs...)` is deprecated, use `", $call, "{", $T, "}(x, StateSpaceSet(y), ε; kwargs...)`")
            $call{$T}(x, StateSpaceSet(y), ε; kwargs...)
        end

        @eval function $call{$T}(x::AbstractMatrix, y::AbstractMatrix, ε; kwargs...)
            @warn string("`", $call, "{", $T, "}(x::AbstractMatrix, y::AbstractMatrix, ε; kwargs...)` is deprecated, use `", $call, "{", $T, "}(StateSpaceSet(x), StateSpaceSet(y), ε; kwargs...)`")
            $call{$T}(StateSpaceSet(x), StateSpaceSet(y), ε; kwargs...)
        end
    end
end
